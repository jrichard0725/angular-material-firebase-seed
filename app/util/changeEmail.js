  ...
  .factory('changeEmail', ['fbutil', '$q', '$rootScope', function(fbutil, $q, $rootScope) {
    return function(password, oldEmail, newEmail, simpleLogin) {
      ...
      function authOldAccount() {
        return userFactories.login(ctx.old.email, password).then(function(user) {
      ...
      function createNewAccount() {
        return userFactories.createAccount(ctx.curr.email, password, ctx.old.name).then(function(user) {
      ...
      function removeOldLogin() {
        var def = $q.defer();
        userFactories.removeUser(ctx.old.email, password).then(function() {
          def.resolve();
        }, function(err) {
          def.reject(err);
        });
      ...
      function authNewAccount() {
        return userFactories.login(ctx.curr.email, password);
      }
    };
  }]);